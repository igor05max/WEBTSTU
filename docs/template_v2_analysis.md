...
Нельзя копировать авторов, DOI, цитирование, биографии, даты поступления,
лицензионный блок или другие данные чужой статьи. Если этих данных нет в
`ARTICLE`, V2 оставляет место/placeholder и ничего не выдумывает. Header/footer
копируется только если из ARTICLE надёжно извлечена короткая строка авторов для
санитизации footer.
...
только известные поля, существующие ID таблиц ARTICLE, boolean-значения и числа
в заданных диапазонах. Текст, XML, незнакомые ключи и выдуманные ID
игнорируются. При timeout, ошибке JSON, недоступности VPN или модели редактор
...
- переносит header/footer shell, PAGE fields и чередование страниц;
