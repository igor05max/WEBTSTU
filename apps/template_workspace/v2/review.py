...
def editorial_findings(article_report, template_report, structure, profile):
    ...
    if profile.style_id:
        languages = {b.get('language') for b in structure.blocks if b.get('detected_role') == 'title'}
        for language, label in [('en', 'английском'), ('ru', 'русском')]:
            if language not in languages:
                warnings.append(f'Не найден титульный блок на {label} языке. Перевод автоматически не создавался.')
    ...
    for role, label in labels.items():
        if role in profile.roles and role not in roles:
            if profile.style_id:
                warnings.append(f'Для стиля JAMT ожидается поле «{label}», но в статье оно не найдено. Добавьте его в исходник при необходимости.')
            else:
